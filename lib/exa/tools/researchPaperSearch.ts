...
toolRegistry["research_paper_search"] = {
  ...
  schema: {
    ...
    numResults: z
      .number()
      .optional()
      .describe("Number of research papers to return (default: 5)"),
    maxCharacters: z
      .number()
      .optional()
      .describe(
  ...
  handler: async ({ query, numResults, maxCharacters }, extra) => {
    ...
    try {
      ...
      if (!response.data || !response.data.results) {
        logger.log(
          "Warning: Empty or invalid response from Exa API for research papers"
        );
        return {
          content: [
            {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Research paper search error: ${
