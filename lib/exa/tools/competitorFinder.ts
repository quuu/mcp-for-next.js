...
toolRegistry["competitor_finder"] = {
  ...
  schema: {
    ...
    numResults: z
      .number()
      .optional()
      .describe("Number of competitors to return (default: 10)"),
  },
  handler: async ({ query, excludeDomain, numResults }, extra) => {
    const requestId = `competitor_finder-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 7)}`;
    ...
    try {
      ...
      if (!response.data || !response.data.results) {
        logger.log(
          "Warning: Empty or invalid response from Exa API for competitor finder"
        );
        return {
          content: [
            {
