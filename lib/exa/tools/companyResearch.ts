...
import { ExaSearchRequest, ExaSearchResponse } from "../types";
...
toolRegistry["company_research"] = {
  ...
  schema: {
    query: z
      .string()
      .describe("Company website URL (e.g., 'exa.ai' or 'https://exa.ai')"),
    subpages: z
      .number()
      .optional()
      .describe("Number of subpages to crawl (default: 10)"),
    subpageTarget: z
      .array(z.string())
      .optional()
      .describe(
  ...
  handler: async ({ query, numResults, subpages, subpageTarget }, extra) => {
    const requestId = `company_research-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 7)}`;
    ...
    try {
      ...
      if (!response.data || !response.data.results) {
        logger.log(
          "Warning: Empty or invalid response from Exa API for company research"
        );
        return {
          content: [
            {
      ...
      logger.log(
        `Found ${response.data.results.length} results about the company`
      );
