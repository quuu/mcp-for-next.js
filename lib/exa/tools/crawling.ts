...
import { ExaCrawlRequest } from "../types";
...
toolRegistry["crawling"] = {
  ...
  handler: async ({ url }, extra) => {
    const requestId = `crawling-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 7)}`;
    ...
    try {
      ...
      const response = await axiosInstance.post("/contents", crawlRequest, {
        timeout: 25000,
      });
      ...
      if (
        ...
        logger.log(
          "Warning: Empty or invalid response from Exa API for crawling"
        );
        return {
          content: [
            {
