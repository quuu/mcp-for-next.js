...
class MentalModelServer {
  ...
  private formatModelOutput(modelData: MentalModelData): string {
    ...
    const border = "─".repeat(
      Math.max(modelName.length + 20, problem.length + 4)
    );
...
class DebuggingApproachServer {
  ...
  private formatApproachOutput(approachData: DebuggingApproachData): string {
    ...
    const border = "─".repeat(
      Math.max(approachName.length + 25, issue.length + 4)
    );
...
// Export function to register tools with an McpServer
export function registerClearThoughtTools(server: McpServer) {
  server.tool(
    ...
    async (args, extra) => thinkingServer.processThought(args)
  );
  ...
  server.tool(
    "mentalmodel",
    {
      modelName: z
        ...
        .describe("The mental model to apply"),
      problem: z.string().describe("The problem to solve"),
      steps: z
        .array(z.string())
        .optional()
        .describe("Steps in applying the model"),
      reasoning: z
        .string()
        .optional()
        .describe("Reasoning behind the model application"),
      conclusion: z
        .string()
        .optional()
        .describe("Conclusion reached through the model"),
    },
    async (args, extra) => modelServer.processModel(args)
  );
  ...
  server.tool(
    "debuggingapproach",
    {
      approachName: z
        ...
        .describe("The debugging approach to apply"),
      issue: z.string().describe("The issue to debug"),
      steps: z
        .array(z.string())
        .optional()
        .describe("Steps in applying the approach"),
      findings: z
        .string()
        .optional()
        .describe("Findings discovered through debugging"),
      resolution: z.string().optional().describe("Resolution of the issue"),
    },
    async (args, extra) => debuggingServer.processApproach(args)
  );
}
